...
//
// ВАЖНО: используем img.decode(), а не просто onload. onload срабатывает,
// как только скачаны байты картинки — но браузер может декодировать её в
// готовый к отрисовке битмап чуть ПОЗЖЕ этого момента (отдельная, не всегда
// синхронная стадия). Раньше здесь ждали только onload, и на практике
// изредка ловили гонку: buildPages() успевал измерить clientHeight ДО того,
// как декодирование реально завершилось — высота страницы получалась
// заниженной, и уже показанный текст визуально вылезал за нижнюю границу
// страницы, а следующая страница из-за этого "перескакивала" на несколько
// слов вперёд (разбивка уже отсчитала эти слова как показанные, хотя
// фактически они вылезали за рамку). img.decode() — это Promise, который
// гарантированно ждёт полного декодирования, а не только скачивания.
function preloadImages(srcList) {
  return Promise.all(srcList.map(src => {
    const img = new Image();
    img.src = src;
    if (typeof img.decode === 'function') {
      return img.decode().catch(() => {}); // не блокируем разбивку, если картинка не нашлась/повреждена
    }
    // Старые браузеры без decode() — прежнее поведение как запасной вариант
