...
-- Аналитика просмотров глав
-- Пишется автоматически при каждом успешном запросе текста главы.
-- utm_source — заполняется только если читатель перешёл по ссылке с UTM-меткой
-- (например, из Telegram-поста с кнопкой, у которой в URL был ?utm_source=telegram).
CREATE TABLE IF NOT EXISTS page_views (
  id SERIAL PRIMARY KEY,
  book_id INTEGER REFERENCES books(id) ON DELETE CASCADE,
  chapter_id INTEGER REFERENCES chapters(id) ON DELETE CASCADE,
  user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,  -- NULL = анонимный читатель
  ip_hash VARCHAR(64),            -- SHA-256 от IP — для подсчёта уникальных, но без хранения сырого IP
  device_type VARCHAR(20),        -- 'mobile' | 'desktop' | 'tablet'
  utm_source VARCHAR(100),        -- 'telegram', 'vk', null ...
  viewed_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_page_views_book_chapter ON page_views(book_id, chapter_id);
